    ...
    <div className="rounded-xl bg-white p-4 shadow-sm dark:bg-gray-800">
      ...
      <div className="space-y-1.5">
        {profiles.map((profile) => (
          <div key={profile.id} className={`flex items-center justify-between rounded-lg px-3 py-2 transition-colors ${activeProfileId === profile.id ? 'bg-blue-50 ring-1 ring-blue-200 dark:bg-blue-900/20 dark:ring-blue-800' : 'bg-gray-50 hover:bg-gray-100 dark:bg-gray-700/50 dark:hover:bg-gray-700'}`}>
            <button onClick={() => handleSwitch(profile)} className="flex flex-1 items-center gap-2 text-left">
              <span className="text-base">{profile.icon}</span>
              <div>
                <p className="text-sm font-medium text-gray-800 dark:text-white">{profile.name}</p>
                <p className="text-[10px] text-gray-400">{profile.goalMl} ml · {t(`activity.${profile.activityLevel}`)}</p>
              </div>
            </button>
            <div className="flex items-center gap-1.5">
              ...
              {profile.id !== 'default' && (
                <button onClick={() => removeProfile(profile.id)} className="rounded p-1 text-gray-400 transition-colors hover:text-red-500">
      ...
      {showAdd ? (
        <div className="mt-3 space-y-2 rounded-lg border border-gray-200 p-3 dark:border-gray-600">
          <input type="text" placeholder={t('profiles.profileName')} value={newName} onChange={(e) => setNewName(e.target.value)} className="w-full rounded-lg border border-gray-200 px-3 py-1.5 text-sm focus:border-blue-400 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white" />
          <input type="number" placeholder={t('profiles.goalMl')} value={newGoal} onChange={(e) => setNewGoal(e.target.value)} className="w-full rounded-lg border border-gray-200 px-3 py-1.5 text-sm focus:border-blue-400 focus:outline-none dark:border-gray-600 dark:bg-gray-700 dark:text-white" />
          <div className="flex gap-2">
            <button onClick={handleAdd} className="flex-1 rounded-lg bg-blue-500 py-1.5 text-xs font-medium text-white hover:bg-blue-600">{t('profiles.add')}</button>
            <button onClick={() => setShowAdd(false)} className="flex-1 rounded-lg border border-gray-200 py-1.5 text-xs text-gray-600 dark:border-gray-600 dark:text-gray-300">{t('profiles.cancel')}</button>
          </div>
        </div>
      ) : (
